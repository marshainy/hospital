...
import com.hospital.form.ForgetPasswordForm;
import com.hospital.form.LoginForm;
import com.hospital.form.UserRegistrationForm;
...
public class LoginCtl extends BaseCtl {
	...
	/*
	 * @RequestMapping(value = "/ForgetPassword", method = RequestMethod.GET) public
	 * String display(@ModelAttribute("form") ForgetPasswordForm form, HttpSession
	 * session, Model model) {
	 * 
	 ...
	 * public String display(Locale locale, @ModelAttribute("form") @Valid
	 * ForgetPasswordForm form, BindingResult bindingResult, Model model) {
	 * 
	 * if (bindingResult.hasErrors()) { return "ForgetPassword"; }
	 * 
	 * UserDTO dto = service.findByLogin(form.getUserName());
	 * 
	 * if (dto == null) { String msg =
	 * messageSource.getMessage("message.forgetpassworderror", null, locale);
	 * model.addAttribute("error", msg); }
	 * 
	 * if(dto!=null){ service.forgetPassword(form.getUserName());
	 * 
	 * 
	 * String msg = messageSource.getMessage("message.success.forgetpassword", null,
